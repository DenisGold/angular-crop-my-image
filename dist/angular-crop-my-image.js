angular.module('angular-crop-my-image', []).directive("myCropper", ['$document', '$window', 'myCropperDataShare', function ($document, $window, myCropperDataShare) {
    return {
        scope: {
            image: "=",
            croppedImage: "=",
        ...
        link: function (scope, element) {
            ...
            var myCropper = (function () {
                function myCropper(canvas, x, y, width, height, keepAspect, touchRadius) {
                    if (x === void 0) {
                        x = 0;
                    }
                    if (y === void 0) {
                ...
                myCropper.prototype.resizeCanvas = function (width, height) {
                ...
                myCropper.prototype.draw = function (ctx) {
                ...
                myCropper.prototype.dragCrop = function (x, y, marker) {
                    var bounds = this.getBounds();
                ...
                myCropper.prototype.enforceMinSize = function (x, y, marker) {
                ...
                myCropper.prototype.dragCorner = function (x, y, marker) {
                    var iX = 0;
                    var iY = 0;
                    var ax = 0;
                    var ay = 0;
                ...
                myCropper.prototype.getSide = function (a, b, c) {
                ...
                myCropper.prototype.sign = function (x) {
                    if (+x === x) {
                        return (x === 0) ? x : (x > 0) ? 1 : -1;
                    }
                    return NaN;
                };
                myCropper.prototype.handleRelease = function (newCropTouch) {
                ...
                myCropper.prototype.handleMove = function (newCropTouch) {
                    var matched = false;
                    for (var k = 0; k < this.currentDragTouches.length; k++) {
                        if (newCropTouch.id == this.currentDragTouches[k].id && this.currentDragTouches[k].dragHandle != null) {
                            ...
                            matched = true;
                            myCropperDataShare.setPressed(this.canvas);
                            break;
                        }
                    }
                    if (!matched) {
                ...
                myCropper.prototype.updateClampBounds = function () {
                    var sourceAspect = this.srcImage.height / this.srcImage.width;
                ...
                myCropper.prototype.getCropBounds = function () {
                ...
                myCropper.prototype.clampPosition = function (x, y) {
                    if (x < this.minXClamp) {
                        x = this.minXClamp;
                    }
                    if (x > this.maxXClamp) {
                ...
                myCropper.prototype.isImageSet = function () {
                    return this.imageSet;
                };
                myCropper.prototype.setImage = function (img) {
                    if (!img) {
                        throw "Image is null";
                    }
                    this.imageSet = true;
                ...
                myCropper.prototype.getCroppedImage = function (fillWidth, fillHeight) {
                    var bounds = this.getBounds();
                    if (!this.srcImage) {
                    ...
                    if (fillWidth && fillHeight) {
                        ...
                        this.ratioW = w / this.srcImage.width;
                        this.ratioH = h / this.srcImage.height;
                        this.cropCanvas.width = Math.max(Math.round(bounds.getWidth() / this.ratioW), 1);
                        this.cropCanvas.height = Math.max(Math.round(bounds.getHeight() / this.ratioH), 1);
                        var offsetH = (this.buffer.height - h) / 2 / this.ratioH;
                        var offsetW = (this.buffer.width - w) / 2 / this.ratioW;
                        this.drawImageIOSFix(this.cropCanvas.getContext('2d'), this.srcImage, Math.max(Math.round((bounds.left) / this.ratioW - offsetW), 0), Math.max(Math.round(bounds.top / this.ratioH - offsetH), 0), Math.max(Math.round(bounds.getWidth() / this.ratioW), 1), Math.max(Math.round(bounds.getHeight() / this.ratioH), 1), 0, 0, Math.max(Math.round(bounds.getWidth() / this.ratioW), 1), Math.max(Math.round(bounds.getHeight() / this.ratioH), 1));
                        this.croppedImage.width = Math.max(Math.round(bounds.getWidth() / this.ratioW), 1);
                        this.croppedImage.height = Math.max(Math.round(bounds.getHeight() / this.ratioH), 1);
                    }
                    else {
                ...
                myCropper.prototype.getBounds = function () {
                ...
                myCropper.prototype.setBounds = function (bounds) {
                ...
                myCropper.prototype.onMouseMove = function (e) {
                ...
                myCropper.prototype.move = function (cropTouch, e) {
                    if (this.isMouseDown) {
                        this.handleMove(cropTouch);
                    }
                };
                myCropper.prototype.getDragTouchForID = function (id) {
                ...
                myCropper.prototype.drawCursors = function (cropTouch, e) {
                    var cursorDrawn = false;
                    if (cropTouch != null) {
                        if (cropTouch.dragHandle == this.center) {
                            myCropperDataShare.setStyle(this.canvas, 'move');
                            cursorDrawn = true;
                        }
                    ...
                    if (!cursorDrawn) {
                        ...
                        if (!didDraw) {
                            myCropperDataShare.setStyle(this.canvas, 'initial');
                        }
                    }
                    if (!didDraw && !cursorDrawn && this.center.touchInBounds(cropTouch.x, cropTouch.y)) {
                        this.center.setOver(true);
                        myCropperDataShare.setOver(this.canvas);
                        myCropperDataShare.setStyle(this.canvas, 'move');
                    }
                    else {
                ...
                myCropper.prototype.drawCornerCursor = function (marker, x, y, e) {
                    if (marker.touchInBounds(x, y)) {
                        ...
                        if (marker.getHorizontalNeighbour().getPosition().x > marker.getPosition().x) {
                            if (marker.getVerticalNeighbour().getPosition().y > marker.getPosition().y) {
                                myCropperDataShare.setOver(this.canvas);
                                myCropperDataShare.setStyle(this.canvas, 'nwse-resize');
                            }
                            else {
                                myCropperDataShare.setOver(this.canvas);
                                myCropperDataShare.setStyle(this.canvas, 'nesw-resize');
                            }
                        }
                        else {
                            if (marker.getVerticalNeighbour().getPosition().y > marker.getPosition().y) {
                ...
                myCropper.prototype.onTouchStart = function (e) {
                    if (crop.isImageSet()) {
                        this.isMouseDown = true;
                    }
                };
                myCropper.prototype.onTouchEnd = function (e) {
                    if (crop.isImageSet()) {
                ...
                myCropper.prototype.drawImageIOSFix = function (ctx, img, sx, sy, sw, sh, dx, dy, dw, dh) {
                ...
                myCropper.prototype.detectVerticalSquash = function (img) {
                ...
                myCropper.prototype.onMouseDown = function (e) {
                    if (crop.isImageSet()) {
                        this.isMouseDown = true;
                    }
                };
                myCropper.prototype.onMouseUp = function (e) {
                    if (crop.isImageSet()) {
                        myCropperDataShare.setReleased(this.canvas);
                        this.isMouseDown = false;
            ...
            var preloaded = function () {
                ...
                crop = new myCropper(canvas, canvas.width / 2 - width / 2, canvas.height / 2 - height / 2, width, height, keepAspect, touchRadius);
            };
...
angular.module('angular-crop-my-image').directive("imgCropperFileread", ['$timeout', function ($timeout) {
    return {
        scope: {
            image: "="
        },
...
angular.module('angular-crop-my-image').directive('imgCropperFilereadCall', function factory() {
    return{
        scope: {
            control: '='
        },
...
angular.module('angular-crop-my-image').factory("myCropperDataShare", function () {
    var share = {};
    var pressed;
    var over;
